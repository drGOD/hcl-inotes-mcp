...
Отправка, ответ, пересылка и создание события повторяют отправку формы браузером: открывается `h_PageUI`, поля заполняются и уходят POST-ом вместе с `%%Nonce`. Успех означает, что iNotes не вернул страницу ошибки. Это не квитанция о доставке.
